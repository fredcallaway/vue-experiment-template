...
- Added a browsable `/demo` section in `core/pages/demo/` that hosts standalone documentation pages (`demo/phases.vue`, `demo/outline.vue`, `demo/error.vue`), and a `Demo` link in the top NavBar. These replace the ad-hoc `EDemo*` components (`EDemoPhases`, `EDemoOutline`, `EDemoError`) that were previously rendered inside the demo experiment; if your project copied those components, delete them and visit `/demo` instead.
